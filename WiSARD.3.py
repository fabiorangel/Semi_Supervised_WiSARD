...
class Testing:
    ...
    def __init__(self):
        self.exp1 = Experiment("new_sts","en")
        self.X, self.y, Xun, self.testing_X, self.testing_y = self.exp1.random_subsampling(0.5, 0.0, 'WiSARD')
        print len(self.X), len(self.testing_X)

        self.w1 = WiSARD(retina_size = len(self.X[0]),
                     num_bits_addr = 8,
                     bleaching = False,
    ...
    def start_test(self):
        ...
        result = [0]*len(self.testing_X)

        time1 = time.time()
        for class_name in classes:
            self.w1.create_discriminator(class_name)
        print 'time to fit', str(time.time() - time1)
        time1 = time.time()
        for i in xrange(len(self.X)):
            self.w1.add_training(self.y[i], self.X[i])
        print 'time to predict', str(time.time() - time1)
        for i in xrange(len(self.testing_X)):
            result[i] = self.w1.classify(self.testing_X[i])
        print 'result: ', self.exp1.SS_WiSARD_eval(result, self.testing_y)

if __name__ == "__main__":
    test = Testing()

    cProfile.run('test.start_test()')